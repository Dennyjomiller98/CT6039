...
<div class="content">
    ...
    <div class="main-body-content">
        <h1><%="Assign Homework To Class"%></h1>
        <br/>
    </div>
    ...
    <form class="reg-form" action="${pageContext.request.contextPath}/servlets/homework/AddHomework" method="POST">
        ...
        <br/>
        <label for="name" class="form-label"><%="Homework Title"%></label>
        <input type="text" name="name" id="name" class="form-control" placeholder="E.g. 'Science Poster Homework'" required/>
        <br/>
        <label for="description" class="form-label"><%="Homework Description"%></label>
        <textarea type="text" name="description" id="description" class="form-control" aria-label="With textarea" placeholder="Further describe the Homework task here..." required></textarea>
        <br/>
        <label for="dueDate" class="form-label"><%="Due Date"%></label>
        <input type="date" name="dueDate" id="dueDate" class="form-control" required/>
        <br/>
        <label for="classSelect" class="form-label"><%="Class to Assign Homework for"%></label>
        <select class="form-control select-css selectpicker classSelect" name="classSelect" id="classSelect" data-live-search="true">
            ...
            <% List<ClassBean> allClasses = (List<ClassBean>) session.getAttribute("allClasses");
                if(allClasses != null) {
                    ...
                    {%>
            <option <% if(allClass.getEventId().equals(selectedClass)){ %> selected <% } %> value="<%=allClass.getEventId()%>"><%=allClass.getName()%></option>
                <%}
            }%>
        </select>
        <br/>
        <div class="body-main-content">
            <br/>
            <a class="btn btn-primary" href=${pageContext.request.contextPath}/servlets/Redirects?location=class-view>&nbsp;Back to Classes&nbsp;</a>
            <input class="btn btn-primary" type="submit" value="Assign Homework">
        </div>
    </form>
</div>
